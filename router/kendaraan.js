...
router.post('/store', upload.single('gambar_kendaraan'), [body('no_pol').notEmpty(), body('nama_kendaraan').notEmpty(), body('id_transmisi').notEmpty().isInt()], (req, res) => {
...
router.put('/update/:no_pol', (req, res) => {
  // Implementasi update kendaraan
  const { no_pol } = req.params;
  const { nama_kendaraan, 
    id_transmisi } = req.body;

  connection.query('UPDATE kendaraan SET nama_kendaraan = ?, id_transmisi = ? WHERE no_pol = ?', [nama_kendaraan, id_transmisi, no_pol], (err, result) => {
    if (err) {
      return res.status(500).json({
        status: false,
        message: 'Server Error',
